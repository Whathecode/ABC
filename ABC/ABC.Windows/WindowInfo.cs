...
namespace ABC.Windows
{
    ...
    {
        private const int MaxClassnameLength = 128; // TODO: Is there an actual maximum class name length?

        private readonly Dictionary<User32.WindowState, WindowState> _windowStateMapping = new Dictionary
        ...
        /// </summary>
        private User32.WindowPlacement GetWindowPlacement()
        {
        ...
        #region Experimental Thumbnail

        internal IntPtr ThumnailHandle;

        public void RegisterThumbnail(IntPtr baseWindow, ref System.Windows.Controls.Image image, Point location, Size size)
        {
            if (ThumnailHandle != IntPtr.Zero)
                DwmApi.DwmUnregisterThumbnail(ThumnailHandle);

            DwmApi.DwmRegisterThumbnail(baseWindow, Handle, out ThumnailHandle);

            if (ThumnailHandle == IntPtr.Zero)
                return;

            var thumbnailProperties = new DwmApi.DwmThumbnailProperties
                {
                    dwFlags = DwmApi.DwmThumbnailProperties.DWM_TNP_VISIBLE +
                              DwmApi.DwmThumbnailProperties.DWM_TNP_OPACITY +
                              DwmApi.DwmThumbnailProperties.DWM_TNP_RECTDESTINATION,
                    opacity = 255,
                    fVisible = true,
                    rcDestination = new DwmApi.RECT((int) location.X, (int) location.Y, (int)size.Width,
                                                    (int)size.Height)
                };

            DwmApi.DwmUpdateThumbnailProperties(ThumnailHandle, thumbnailProperties);
        }
