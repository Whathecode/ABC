...
namespace ABC.Applications.Persistence
{
	...
	public abstract class AbstractPersistenceProvider : AbstractDisposable
	{
		...
		public List<PersistedApplication> Suspend( List<Window> windows )
		{
			var persistedApplications = (
				...
				let persistor = GetPersistenceProviders().FirstOrDefault( p => new TargetProcess( p.ProcessName, p.CompanyName, p.TargetVersion ).Matches( process ) )
		...
		public void Resume( List<PersistedApplication> persistedStates )
		{
			foreach ( var persistedState in persistedStates )
			{
				...
				if ( persistor != null )
				{
					...
					PluginHelper.SafePluginInvoke( persistor, p => p.Resume( s.ApplicationPath, s.Data ) );
				}
			}
		}
		...
		public List<Type> GetPersistedDataTypes()
		{
			return GetPersistenceProviders()
				.Select( p => PluginHelper.SafePluginInvoke( p, pi => pi.GetPersistedDataType() ) )
				.ToList();
		}
